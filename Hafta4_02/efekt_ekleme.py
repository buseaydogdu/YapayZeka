...
while video.isOpened():
    ...
    yeniDizi = np.hsplit(alinanGoruntu, 2)
    # print(f'Yükseklik: {yukseklik}, Genişlik:{genislik}, renk:{renk}')

    yukseklik, genislik, renk = yeniDizi[1].shape

    yeniResim= np.full([yukseklik,genislik,3], [1990,1900,1990], dtype=np.uint8)
    yeniDizi[1] = np.add(yeniDizi[1],yeniResim)
    gosterilecekSekli = np.concatenate((yeniDizi[0],yeniDizi[1]),axis=1)

    # niResim,yeniDizi[2]),axis=1)
