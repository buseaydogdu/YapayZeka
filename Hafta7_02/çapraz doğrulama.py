...
cvs = cross_val_score(model, X_train, y_train, cv=2, scoring="neg_mean_squared_error")
cvsm = np.mean(-cross_val_score(model,X_train, y_train, cv=2, scoring ="neg_mean_squared_error"))
cvsm = mse = np.mean(-cross_val_score(model,X_train, y_train, cv=2, scoring ="neg_mean_squared_error"))
cvsmr = rmse = np.sqrt(np.mean(-cross_val_score(model,X_train, y_train, cv=2, scoring ="neg_mean_squared_error")))
print("\ncross_val_score                : ",cvs)
print("\ncross_val_score mean mse       : ",mse)
print("\ncross_val_score mean root rmse : ",rmse)
# Sınama seti ile cros validation karşılaştırması
vs = rmse = np.sqrt(np.mean(-cross_val_score(model,X, y, cv=2, scoring ="neg_mean_squared_error")))
